...
Whence is a proxy: it should secure the operator's *own* MCP servers, not ship
...
1. the operator declares their servers (``WHENCE_MCP_SERVERS``);
2. Whence connects to each and asks what tools it has (``tools/list``);
3. the routing table is built from what was discovered, not from a constant.

That is what makes Whence a **cross-server** information-flow layer: it is the
...
* **Poisoning** — discovered definitions are shield-scanned before being served
  to an agent (:mod:`whence.catalogue`, applied by the caller's preflight).
...
from whence.catalogue import CatalogueIntegrityError, detect_shadowing
...
def parse_servers(raw: str | None) -> tuple[DownstreamServer, ...]:
    """Parse ``WHENCE_MCP_SERVERS`` into validated server specs.
    ...
    try:
        ...
        raise DownstreamConfigError(
            f"WHENCE_MCP_SERVERS is not valid JSON: {exc}"
        ) from exc
    ...
    if isinstance(data, dict):
        ...
        raise DownstreamConfigError(
            "WHENCE_MCP_SERVERS must be a JSON array or object, "
    ...
    if not servers:
        raise DownstreamConfigError("WHENCE_MCP_SERVERS declared no servers")
...
def _downstream_http_client() -> httpx.AsyncClient:
    """The HTTP client Whence uses for downstream MCP connections.
...
async def open_downstream_session(url: str, stack: AsyncExitStack) -> SessionLike:
    """Open ONE downstream MCP session — the single way Whence connects out.

    Both downstream paths route through here (operator-declared
    ``WHENCE_MCP_SERVERS`` and the legacy ``WHENCE_TOOLS_*_URL`` deployment
