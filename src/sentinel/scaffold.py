...
The onboarding problem: SENTINEL default-denies any tool with no policy entry —
...
servers and are therefore attacker-controlled (see :mod:`sentinel.catalogue`).
...
# hunting for the grammar. These mirror sentinel.authorization.parser exactly.
...
def scaffold_policy(
    ...
    :func:`~sentinel.authorization.policy.load_policy`) in which every tool is
    explicitly denied. Applying it is a no-op security-wise; editing it is how the
    operator grants access.
    """
    lines: list[str] = [
        "# SENTINEL policy scaffold - generated from your discovered MCP tools.",
        "#",
        "# EVERY tool below is DENIED. That mirrors SENTINEL's default-deny (a tool",
...
def _main() -> int:  # pragma: no cover - thin CLI wrapper
    ...
    Usage::
        ...
        SENTINEL_MCP_SERVERS='[{"name":"gh","url":"https://..."}]' \\
            python -m sentinel.scaffold > my-policy.yaml
    """
    import asyncio
    import sys
    ...
    if not servers:
        print(
            "No downstream servers declared. Set SENTINEL_MCP_SERVERS, e.g.\n"
            '  SENTINEL_MCP_SERVERS=\'[{"name":"gh","url":"https://mcp.example/gh"}]\'',
            file=sys.stderr,
        )
        return 2
