...
from whence.authorization.registry import DEFAULT_TENANT, PolicyRegistry, ReloadResult
from whence.classifier import AttackClassifier
from whence.classifier.attack_classifier import BlockedAttempt
from whence.config import Settings, get_settings
from whence.control.capabilities import mode_summary
from whence.control.events import BroadcastStore, EventBus
...
def _build_registry(settings: Settings) -> PolicyRegistry:
    ...
    if not path.is_file():
        raise ValueError(
            f"policy file not found: {path} (set WHENCE_POLICY_FILE / `policy:` "
            "in whence.yaml, or run `whence scaffold` to generate one)"
        )
...
def _default_store(settings: Settings) -> ForensicStore:
    ...
    :class:`SqliteForensicStore` under ``$WHENCE_DATA_DIR`` (or ``./var``), which
    ...
    data_dir = Path(os.environ.get("WHENCE_DATA_DIR", "var"))
    return SqliteForensicStore(data_dir / "whence.db")
...
class RunManager:
    ...
    ``${WHENCE_DATA_DIR:-./var}/whence.db`` so runs survive a restart. Tests
    ...
    def __init__(
        ...
        *,
        ...
        # Mode is driven by WHENCE_DEMO_MODE (settings) unless overridden, so the
    ...
    def _select_driver(self, build: ScenarioBuild) -> Any:  # noqa: ANN401 - AgentDriver
        ...
        """
        from whence.demo.llm_driver import default_agent_driver
    ...
    async def run_baseline(self, build: ScenarioBuild) -> dict[str, Any]:
        """Run a scenario WITHOUT Whence in front and return its observable result.
        ...
        no spans are emitted (there is no Whence in this path). The caller —
