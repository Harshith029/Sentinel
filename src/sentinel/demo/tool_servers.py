...
* ``send_email(...)``          → appends to an observable OUTBOX SINK; nothing
  is ever actually sent. (When SENTINEL blocks the call, this tool never runs,
...
the tools SENTINEL allowed actually ran downstream.
"""
...
# A clearly-synthetic record. The SSN/api_key are sentinels we assert NEVER leak.
