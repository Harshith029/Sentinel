...
wired in DEMO MODE — it needs Whence's MCP server exposed over a real HTTP
...
from whence.control.events import BroadcastEvent
...
from whence.forensics.replay import TraceReplay
...
class CustomRunRequest(BaseModel):
    ...
    Fields mirror :class:`~whence.demo.scenario.CustomScenarioSpec`. Strings
...
SESSION_COOKIE: Final[str] = "whence_session"
...
def _expected_token() -> str | None:
    ...
    ``WHENCE_API_TOKEN`` and restarting takes effect. Deliberately tiny: the
...
def _presented_credential(request: Request) -> str:
    ...
    * a ``whence_session`` cookie — browsers. This is not decoration: the
...
def require_auth(request: Request) -> None:
    ...
    ``WHENCE_ALLOW_ANONYMOUS``, the service refuses to answer rather than
    ...
    if expected is None:
        ...
        raise HTTPException(
            ...
            detail=(
                "Whence is not configured for authentication. Set "
                "WHENCE_API_TOKEN to a secret value, or set "
                "WHENCE_ALLOW_ANONYMOUS=1 to run with NO authentication "
...
def create_app(
    ...
    *,
    ...
    ``enable_mcp_gateway`` (default: the ``WHENCE_ENABLE_MCP_GATEWAY`` setting,
    off) mounts Whence's proxy as a real streamable-HTTP MCP server at ``/mcp``
    ...
    gateway = WhenceGateway(mgr) if enable_mcp_gateway else None
    ...
    if gateway is not None:
        ...
        app = FastAPI(title="Whence control plane", lifespan=_lifespan)
    else:
        app = FastAPI(title="Whence control plane")
    ...
    async def custom_baseline(body: CustomRunRequest) -> dict[str, Any]:
        """Run the same custom attack WITHOUT Whence and report what arrived.
    ...
    async def attack_baseline(scenario: str) -> dict[str, Any]:
        """Run a named scenario WITHOUT Whence — the "before" half of the demo."""
        try:
    ...
    async def downstream() -> dict[str, Any]:
        """What Whence is protecting: connected servers, discovered tools, defenses.
    ...
    async def get_audit(
        ...
        if format.lower() == "jsonl":
            ...
            return PlainTextResponse(
                ...
                headers={
                    "Content-Disposition": (
                        f'attachment; filename="whence-{run_id}.jsonl"'
                    ),
                },
            )
    ...
    if gateway is not None:
        ...
        # here; the gateway runs each session through a per-session WhenceProxy.
...
def create_gateway_app() -> FastAPI:
    ...
    external MCP client can be put in front of Whence. The bare
