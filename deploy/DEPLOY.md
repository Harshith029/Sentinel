...
|---|---|
| **Topology / thesis** | `sentinel-proxy` has `ingress.external = true` (the ONLY public endpoint). The three `sentinel-tools-*` apps have `ingress.external = false` — **no external ingress**, reachable only inside the ACA environment, i.e. only by SENTINEL. |
