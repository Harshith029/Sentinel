"""Whence MCP-over-HTTP gateway (the wire transport).

Phases 4–8 proved the pipeline but connected the agent to Whence over an
...
This module closes that gap. It exposes Whence's proxy as a real
**streamable-HTTP MCP server** at ``/mcp``. Any MCP client that can reach the URL
registers Whence as its tool server and is then secured by the IDENTICAL
...
:meth:`~whence.mcp_proxy.proxy.WhenceProxy.handle_call`.

Topology::

    external MCP client (session A) ─┐
                                     ├─ HTTP/MCP ─▶ Whence gateway ─▶ per-session
    external MCP client (session B) ─┘                                  WhenceProxy
                                                                            │
                                                  shared (mock) downstream tool servers

One front :class:`~mcp.server.lowlevel.Server` accepts every HTTP session; each
MCP **session** gets its OWN :class:`~whence.mcp_proxy.proxy.WhenceProxy`
...
* ``WHENCE_MCP_SERVERS`` — the operator's own MCP servers (the product path);
* ``WHENCE_TOOLS_{WEB,EMAIL,RECORDS}_URL`` — the remote example topology the
...
(:func:`~whence.downstream.open_downstream_session`), so connect timeout,
...
from whence.authorization.registry import DEFAULT_TENANT
from whence.catalogue import CatalogueFinding, CatalogueMonitor
from whence.config import get_settings
from whence.control.manager import RunManager
from whence.demo.preflight import preflight
...
from whence.mcp_proxy.content import mcp_error
...
_LOG: Final[logging.Logger] = logging.getLogger("whence.control.mcp_gateway")
_GATEWAY_NAME: Final[str] = "whence"
...
def _request_authorized(scope: Scope) -> bool:
    ...
    ``WHENCE_ALLOW_ANONYMOUS``, this refuses. An unauthenticated /mcp is an
...
class WhenceGateway:
    """A streamable-HTTP MCP server that fronts the Whence pipeline.
    ...
    def describe(self) -> dict[str, Any]:
        """Operator-facing view of what Whence is actually protecting.
    ...
    async def __aenter__(self) -> WhenceGateway:
    ...
    async def _enter(self) -> WhenceGateway:
    ...
    async def _connect_declared_downstream(
        ...
        hardcoded map, so Whence proxies servers it has never seen. Fails closed
    ...
    async def _connect_remote_downstream(self, urls: dict[str, str]) -> None:
        ...
        This is the real-deployment path: Whence is the only thing that reaches
        ...
        if missing:
            raise RuntimeError(
                ...
                "WHENCE_TOOLS_{WEB,EMAIL,RECORDS}_URL for all of them, or unset "
    ...
    async def handle_asgi(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        When ``WHENCE_API_TOKEN`` is set, the wire endpoint requires a matching
    ...
    def _build_front_server(self) -> Server:
        ...
        async def _call_tool(
            ...
            if proxy is None:
                ...
                return mcp_error(
                    "Whence: at session capacity; refusing a new agent session "
    ...
    async def _proxy_for_current_session(self) -> WhenceProxy | None:
        """The :class:`WhenceProxy` bound to the CURRENT MCP session.
    ...
    async def _proxy_for_session(self, session: ServerSession) -> WhenceProxy | None:
