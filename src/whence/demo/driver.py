...
* :class:`ScriptedAgentDriver` — a deterministic transcript that GUARANTEES the
  forbidden tool call is proposed, so Whence deterministically acts. This is
...
class AgentDriver(Protocol):
    """Whatever proposes tool calls to Whence over an MCP client session."""
...
class _DirectAgentClient:
    ...
    The "without Whence" baseline runs the same scripted driver against the
    same downstream tool servers, but routes every ``call_tool`` directly to
    the :class:`~whence.mcp_proxy.router.DownstreamConnection`. No spans are
    emitted — there is no Whence in this path, which IS the point: a viewer
