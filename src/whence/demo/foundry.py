...
pipeline. In real-cloud mode the topology is::
    ...
    Foundry Agent Service  --MCP/HTTP-->  Whence (only reachable endpoint)
                                              --MCP-->  isolated tool servers

i.e. the agent reaches every tool ONLY through Whence, registered as the
agent's MCP tool server. :func:`build_whence_mcp_tool` produces that
...
run — it requires (a) Whence's MCP server exposed over a real HTTP transport at
``WHENCE_MCP_URL`` and (b) a live Foundry project + credentials. The stage demo
...
# The tools Whence proxies. Enumerated explicitly at registration (§6): we do
...
def build_whence_mcp_tool(
    *,
    server_url: str,
    allowed_tools: Sequence[str] = PROXIED_TOOLS,
    require_approval: str = "never",
    server_label: str = "whence",
) -> MCPTool:
    """Build the ``MCPTool`` registration that points a Foundry agent at Whence.
...
def foundry_registration_summary(*, server_url: str) -> dict[str, Any]:
    """Inspectable description of how Whence is registered with Foundry.
    ...
    summary: dict[str, Any] = {
        ...
        "note": (
            "Agent reaches every tool ONLY through Whence; tool servers have no "
            "external ingress. SDK: azure.ai.projects.models.MCPTool."
        ),
    }
    try:
        tool = build_whence_mcp_tool(server_url=server_url)
