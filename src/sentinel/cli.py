"""``sentinel`` — the command-line interface.

This is the product surface. SENTINEL is infrastructure you point at your own MCP
...
Commands::
    ...
    sentinel init                 write a starter sentinel.yaml
    sentinel check                validate config, connect, vet catalogues (no serving)
    sentinel scaffold             print a starter policy for your discovered tools
    sentinel serve                run the proxy
...
The config file (default ``sentinel.yaml``)::
...
DEFAULT_CONFIG = "sentinel.yaml"

STARTER_CONFIG = """\
# SENTINEL configuration. Point it at YOUR MCP servers.
#
# SENTINEL ships no tools of its own: it sits between your agent and your
...
def apply_config(config: dict[str, Any]) -> None:
    ...
    for key, (env_var, serialize) in _ENV_MAP.items():
        ...
        # `sentinel init`), not "explicitly declare nothing" — exporting it would
...
def cmd_scaffold(args: argparse.Namespace) -> int:
    ...
    reset_settings_cache()
    from sentinel.scaffold import _main as scaffold_main
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_init = sub.add_parser("init", help="write a starter sentinel.yaml")
