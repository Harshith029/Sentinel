"""Live-LLM agent driver — a REAL model (or a faithful offline stand-in) drives Whence.

Every earlier phase used a :class:`~whence.demo.driver.ScriptedAgentDriver`: a
...
honest and reproducible, but it leaves one question open: *can Whence stop a
...
execute its tool calls THROUGH Whence, feed the results (including any Whence
...
real ``/mcp`` connection (Phase 9), so "put a real agent behind Whence over the
...
from whence.config import Settings, get_settings
from whence.mcp_proxy.content import result_text

if TYPE_CHECKING:
    from whence.demo.driver import AgentDriver
...
class LLMAgentDriver:
    """An :class:`~whence.demo.driver.AgentDriver` that runs a real tool-use loop.

    Provider-agnostic: hand it any :class:`ModelClient`. It proposes whatever the
    model proposes — Whence is the backstop that decides what actually executes.
    """
    ...
    async def run(self, client: ClientSession) -> list[mcp_types.CallToolResult]:
        ...
        for _ in range(self._max_steps):
            ...
            # each call THROUGH Whence and feed the result back as a tool message.
            messages.append(
                {
                    "role": "assistant",
...
def default_agent_driver(
    *,
    ...
    decides the actions and Whence is the backstop. With no credential it degrades
...
class SusceptibleStubModel:
    ...
    2. Second turn → READ the fetched page from the conversation; if it contains an
       ...
       model being "tricked" by the content — the action Whence must stop.
    3. Third turn → on seeing Whence's block (or success) in the tool results,
       stop with a short summary.

    The point is to prove the loop carries real tool data to the model and that
    Whence blocks the *induced* action. Real susceptibility is the
