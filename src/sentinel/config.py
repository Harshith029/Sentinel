...
class Settings(BaseModel):
    """Immutable view of SENTINEL's runtime configuration.
    ...
    demo_mode: bool = Field(
        description=(
            "Spec §10: True iff SENTINEL is running in DEMO MODE (deterministic "
    ...
    azure_cosmos_database: str = "sentinel"
    ...
    sentinel_mcp_url: str = "http://localhost:8765/mcp"
    ...
    sentinel_tools_web_url: str | None = None
    sentinel_tools_email_url: str | None = None
    sentinel_tools_records_url: str | None = None

    enable_mcp_gateway: bool = Field(
        default=False,
        description=(
            "When True, the control plane also exposes SENTINEL's proxy as a real "
            ...
            "entrypoint / SENTINEL_ENABLE_MCP_GATEWAY=1 turns it on."
        ),
    )
    ...
    def tool_server_urls(self) -> dict[str, str]:
        ...
        """
        configured = {
            "web": self.sentinel_tools_web_url,
            "email": self.sentinel_tools_email_url,
            "records": self.sentinel_tools_records_url,
        }
...
def _read_settings_from_env() -> Settings:
    return Settings(
        demo_mode=_parse_bool_env("SENTINEL_DEMO_MODE", default=True),
        ...
        azure_cosmos_database=os.environ.get("AZURE_COSMOS_DATABASE", "sentinel"),
        ...
        sentinel_mcp_url=os.environ.get("SENTINEL_MCP_URL", "http://localhost:8765/mcp"),
        sentinel_tools_web_url=os.environ.get("SENTINEL_TOOLS_WEB_URL") or None,
        sentinel_tools_email_url=os.environ.get("SENTINEL_TOOLS_EMAIL_URL") or None,
