...
* Per agent, keep the last ``window_size`` tool TRANSITIONS ``(from_tool → to_tool)``
  (the first tool of a trace transitions from a ``<start>`` whence).
...
from whence.forensics.events import AgentQuarantined, ToolBlocked, TrustUpdated
from whence.forensics.span import Span
from whence.observability import log_quarantined
from whence.trust.config import MAX_SCORE, MIN_SCORE, TrustConfig

# The whence "source" for the first tool in a trace (it is never a target, so
