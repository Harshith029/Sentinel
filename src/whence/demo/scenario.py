...
from whence.demo.driver import AgentDriver, ScriptedAgentDriver, Step, ToolInvocation
...
async def run_demo_session(
    ...
    *,
    ...
    skip_whence: bool = False,
    real_web: bool = False,
) -> DemoResult:
    """Run ``driver`` against the single Whence pipeline and return the trace.
    ...
    async with AsyncExitStack() as stack:
        ...
        if skip_whence:
            # "Without Whence" baseline: drive the SAME tool servers directly,
            ...
            # Whence in this path). DO NOT call this in production code; it
            # exists only so the dashboard can render the side-by-side.
            from whence.demo.driver import (  # noqa: PLC0415 - tightly scoped
                _DirectAgentClient,
            )
            ...
            # The agent (driver) connects to Whence ONLY.
    ...
    rep = await replay(store, trace_id) if not skip_whence else _empty_replay(trace_id)
    return DemoResult(
        trace_id=trace_id,
        store=store,
        replay=rep,
...
def _empty_replay(trace_id: str) -> TraceReplay:
    """A no-span TraceReplay for the ``skip_whence`` baseline.

    The baseline path emits nothing through Whence by construction (no proxy,
...
class CustomScenarioSpec:
    ...
    Whence's authorization-by-provenance is what decides whether the third
...
def build_custom_scenario(spec: CustomScenarioSpec) -> ScenarioBuild:
    ...
    and the allowed-domains config the policy is evaluated against. Whence's
